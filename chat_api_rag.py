...
Versão focada apenas no RAG da Scoras Academy
"""
...
app = FastAPI(
    title="Scoras Chatbot API - RAG Version",
    description="API para o chatbot da Scoras Academy com RAG integrado",
    version="1.0.0-rag"
)
...
async def root():
    ...
    return {"message": "Scoras Chatbot API - RAG Version funcionando!", "status": "online"}
...
async def chat_simple_rag(req: ChatRequest):
    """Endpoint de chat com RAG integrado."""
    try:
                     ...
                     if rag_results:
                         ...
                         if rag_snippets:
                             rag_context = (
                                 ...
                                 "\n\n".join(rag_snippets) +
                                 "\n\n**IMPORTANTE**: Você DEVE usar APENAS as informações acima. NÃO invente módulos, conteúdos ou detalhes que não estejam listados. Se não houver informação suficiente, diga que é necessário acessar o site."
                             )
                             used_rag = True
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 Iniciando Scoras Chatbot API - RAG Version")
    print("📍 Endpoint: http://localhost:8000/chat-simple")
    print("🔍 RAG integrado para Scoras Academy")
