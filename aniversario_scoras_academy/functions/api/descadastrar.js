...
async function processar(request, env) {
  ...
  if (!r.ok) {
    return { ok: false, titulo: 'Não foi possível concluir agora', msg: `Houve uma falha ao processar o descadastro de <strong>${email}</strong>. Escreva para <a href="mailto:suporte@scorasacademy.com.br" style="color:#33d9d5;">suporte@scorasacademy.com.br</a> que fazemos manualmente.` };
  }

  return { ok: true, titulo: 'Pronto, você foi descadastrado', msg: `<strong>${email}</strong> não vai mais receber nossos e-mails. Se foi engano, é só se cadastrar de novo no site.` };
}
